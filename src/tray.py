"""System tray icon for background (gesture) mode.
    ...
    if sys.platform == "win32":
        try:
            ...
            command = (
                ...
                f"('{quoted}', 3, 'Gesture Control', 0x40)"
            )
            subprocess.Popen(
...
def _current_color(state) -> tuple[int, int, int]:
    """Pick the icon color: green while detection is active, red while paused."""
...
def create_tray_icon(listener, controller, state=None, verbose: bool = False):
    ...
    draw_icon = _draw_camera
    noun = "Detection"

    icon = pystray.Icon("gesture-controlled-media-player")
    ...
    def _toggle_listening(icon_item=None, item=None) -> None:
        ...
        _show_popup(f"Detection {'resumed' if listening() else 'paused'}")
        _refresh_image()
